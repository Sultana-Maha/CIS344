...
create procedure accountTransactions(IN accountID int) select * from Transactions where accountID = accountID;
call accountTransactions(1);

DELIMITER //
create procedure deposit(IN accountID int, IN amount decimal(6,2))
begin
  start transaction;
...
call deposit (1, 350.00);

DELIMITER //
create procedure withdraw(IN accountID int, IN amount decimal(6,2))
begin
  start transaction;
